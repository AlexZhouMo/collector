...
interface TreeMoveOpts {
  tree: TreeNode;
  /** 标题（含 <b> 等 HTML，调用方自行 esc 动态部分）。 */
  titleHtml: string;
  /** 禁选谓词（虚拟根/当前位置/自身子树）。 */
  isDisabled: (path: string) => boolean;
  /** 节点标签 HTML，返回 "" 表示无。 */
  nodeTag?: (path: string) => string;
  /**
   * 确认（内部做 api 调用 + 成功 showToast），成功 resolve 后由公共逻辑 close + onMoved；
   * 抛错则公共逻辑 showToast(error)。参数 selectedPath 为当前选中目标 path。
   */
  onConfirm: (selectedPath: string) => Promise<void>;
  onMoved: () => void;
}

/**
 * 「移动到目标文件夹」模态弹窗的公共实现（内部，不导出）。
 * openMoveDialog / openFolderMoveDialog 通过 opts 参数化 4 处差异后共用此实现。
 */
function openTreeMoveDialog(opts: TreeMoveOpts): void {
  const { tree, titleHtml, isDisabled, nodeTag, onConfirm, onMoved } = opts;
  ...
  const render = () => {
    ...
    const renderNode = (node: TreeNode, depth: number): string => {
      ...
      const tag = nodeTag ? nodeTag(node.path) : "";
    ...
    dialog.innerHTML = `
      <div class="move-title">${titleHtml}</div>
...
export function openMoveDialog(
  ...
  openTreeMoveDialog({
    tree,
    titleHtml: `移动 <b>「${esc(displayTitle(item.title))}」</b> 到…`,
    isDisabled: (p) => isVirtualRoot(p) || isCurrent(p),
    nodeTag: (p) => (isCurrent(p) ? `<span class="move-tag">当前位置</span>` : ""),
...
export function openFolderMoveDialog(opts: {
  ...
  openTreeMoveDialog({
    tree,
    titleHtml: `移动文件夹 <b>「${esc(folderName)}」</b> 到…`,
    isDisabled,
    nodeTag: (p) => (p === sourcePathInTree ? `<span class="move-tag">当前文件夹</span>` : ""),
    onConfirm: async (selected) => {
      const name = ((): string => {
        if (selected === tree.path) return tree.name;
        const n = findNode(tree, selected);
