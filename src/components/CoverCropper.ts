 ...
 */
export function openCoverCropper(srcPath: string, onDone: (coverPath: string) => void): void {
  ...
  modal.querySelector<HTMLButtonElement>('[data-b="ok"]')!.onclick = async () => {
    ...
    try {
      const cover = await api.importCoverCropped(srcPath, rect.x, rect.y, rect.w, rect.h);
      close();
      onDone(cover);
    } catch (err) {
